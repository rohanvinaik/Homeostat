...
**Warning 2 — "n=1" names two different things, and only one of them is a limit.** The engine is a **zero-time, per-individual read** (Peitho on one body, §3.4): it settles *this* person's own deviations from their own baseline into the most parsimonious mechanism that explains their presentation, and that read is a legitimate mechanism **hypothesis for that person**. N=1 is the operating mode, not a weakness — the engine needs no population, no oracle ensemble, and no cross-source agreement to run. What n=1 **cannot** do is the separate, downstream act of taking one person's settled mechanism and declaring it the **universal definition of a condition** ("this *is* what AuDHD / inflammation is"). That — and only that — is the overclaim §11 guards against. The population data, cross-source convergence, and the σ-ensemble (§5.10, §6) belong to that generalization/validation question; they are **not** preconditions for the per-individual read, and treating them as the method (rather than as a later, optional check on whether a per-person read generalizes) is the exact drift that keeps burying the engine.
...
A per-person read is a legitimate mechanism **hypothesis for that individual** — that is exactly what the engine produces, computed zero-time from their own data, and it is valid as such. What it is *not* is proof that the hypothesis holds as a **population-level law, or as a redefinition of a condition**. Establishing *that* is a separate claim, and I_ind (§5.10) — the information latent across a cohort that no single read supports — is why one person cannot carry *it*. So the distinction is sharp: the engine **runs on n=1 by construction**; the n=1 caveat is only about the downstream over-generalization — *do not declare your own settled mechanism the universal definition of AuDHD / inflammation / anything* — never about whether the per-person mechanism may be computed at all. (§11.1's "sampling-frame specification" is a *third* use: pointing the index case at a population **study design** via the E/I/R cline, §7 — also separate from the engine.)
